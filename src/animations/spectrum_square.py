...
class SpectrumSquareAnimation():
	name = 'spectrum_square'
			...
			for y in range(0, int(self.matrix.width)):
				...
				if amplitude > y:
					...
					if x >= self.min_display and x < self.max_display and y >= self.min_display and y < self.max_display:
						...
						red = round(bass_color/1.5)
						green = round(y_col/4)
						blue = round(x_col/4)
						# reversed_Y = (self.matrix.height - 1) - y
						self.offset_canvas.SetPixel(rot_x + self.cent_x, rot_y + self.cent_y, red, green, blue)
					# else:
